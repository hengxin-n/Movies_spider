...
def saveData2DB(datalist):
    conn = pymysql.connect(host='localhost',user='root',password='111111',db='maoyandb',charset='utf8')
    cur = conn.cursor()
...
def init_db():
    ...
    '''   #创建数据表
    coon = pymysql.connect(host='localhost',user='root',password='111111',db='maoyandb',charset='utf8')       #如果存在就是连接,如果不存在就是创建
    cursor = coon.cursor()
    cursor.execute(sql)
    coon.commit()  #提交
    coon.close()  #关闭数据库
